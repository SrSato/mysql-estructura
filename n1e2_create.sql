...
CREATE TABLE IF NOT EXISTS `pizzeria`.`personas` (
  `personas_id` INT(11) NOT NULL AUTO_INCREMENT COMMENT 'PK autoincremental. No tocar.',
  `personas_nombre` VARCHAR(25) NOT NULL COMMENT 'Nombre propio de la persona.',
  `personas_apellidos` VARCHAR(45) NOT NULL,
  `personas_tfno` VARCHAR(9) NOT NULL COMMENT 'Teléfono de cada persona. Se puede repetir, pero debe existir. ',
  `direcciones_direcciones_id` INT(11) NOT NULL,
  PRIMARY KEY (`personas_id`),
  UNIQUE INDEX `personas_id_UNIQUE` (`personas_id` ASC),
  INDEX `fk_personas_direcciones1_idx` (`direcciones_direcciones_id` ASC),
...
CREATE TABLE IF NOT EXISTS `pizzeria`.`clientes` (
  ...
  INDEX `fk_clientes_personas1_idx` (`personas_personas_id` ASC),
...
CREATE TABLE IF NOT EXISTS `pizzeria`.`direcciones` (
  `direcciones_id` INT(11) NOT NULL AUTO_INCREMENT COMMENT 'PK autoincremental. No tocar.',
  `direcciones_calle` VARCHAR(45) NOT NULL,
  `direcciones_num` INT(11) NOT NULL,
  `direcciones_piso` VARCHAR(2) NOT NULL,
  `direcciones_puerta` VARCHAR(2) NOT NULL,
  `direcciones_cp` VARCHAR(5) NOT NULL COMMENT 'Código postal. No puede ser nulo.',
  `localidades_localidades_id` INT(11) NOT NULL COMMENT 'FK para localidades',
  PRIMARY KEY (`direcciones_id`),
  UNIQUE INDEX `direcciones_id_UNIQUE` (`direcciones_id` ASC),
  INDEX `fk_direcciones_localidades1_idx` (`localidades_localidades_id` ASC),
...
CREATE TABLE IF NOT EXISTS `pizzeria`.`tiendas` (
  ...
  INDEX `fk_tiendas_direcciones1_idx` (`direcciones_direcciones_id` ASC),
